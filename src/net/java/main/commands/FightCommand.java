...
public class FightCommand implements Command {
    ...
    @Override
    public String execute(String[] args) throws NotEnoughEnergyException, InvalidPositionException {

        List<Unit> killedUnit = new ArrayList<>();
        List<Unit> allLiveUnits = this.unitRepository.findAll()
                .stream().filter(unit -> unit.getHealthPoints() > 0)
                .collect(Collectors.toList());

        if (allLiveUnits.size() < 2) {
            return "No enough spawn for fight!";
        }

        StringBuilder result = new StringBuilder();

        for (Unit unit : allLiveUnits) {
            Position currentUnitPosition = unit.getPosition();
            int range = unit.getRange();
            List<Unit> targets = this.battleground
                    .getUnitsInRange(currentUnitPosition.getX(), currentUnitPosition.getY(), range);
            ...
            if (spell.getDamage() > target.getDefencePoints()) {
                ...
                result.append(String.format("%s cast %s spell and did %d damage",
                        unit.getName(), spell.getClass().getSimpleName(),
                        spell.getDamage())).append(System.lineSeparator());
